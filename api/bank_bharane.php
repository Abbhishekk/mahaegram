...
try {
    ...
    if ($operation == 'POST') {
        ...
        handleDeleteRequest();
    } elseif ($operation == 'GET' && isset($_GET['getBanksByPlan'])) {
        handleGetBanksByPlan();
    }elseif ($operation == 'GET' && isset($_GET['getAccountBalance'])) {
...
function handlePostRequest() {
    ...
    $required = ['plan_name', 'date', 'bank_name', 'payer_name', 'amount_to_pay', 'shera'];
    ...
    // Prepare data
    $record_id = !empty($_POST['bank_bharane_id']) ? (int)$_POST['bank_bharane_id'] : null;
    $plan_name = $fun->sanitize($_POST['plan_name']);
    $financial_year = $fun->sanitize($_POST['financial_year']);
    $date = $fun->sanitize($_POST['date']);
    $bank_id = (int)$_POST['bank_name'];
    $payer_name = $fun->sanitize($_POST['payer_name']);
    $account_balance = !empty($_POST['account_balance']) ? (float)$_POST['account_balance'] : 0;
    $balance_inhand = !empty($_POST['balance_inhand']) ? (float)$_POST['balance_inhand'] : 0;
    $amount_to_pay = (float)$_POST['amount_to_pay'];
    $total_amount = (float)$_POST['total_amount'];
    $shera = $fun->sanitize($_POST['shera']);

    if ($record_id) {
        // Update existing record
        $query = "UPDATE bank_bharane SET 
                 plan_name = ?, financial_year = ?, date = ?, 
                 bank_id = ?, payer_name = ?, account_balance = ?,
                 balance_inhand = ?, amount_to_pay = ?, total_amount = ?, shera = ?
                 WHERE id = ? AND district_code = ? and panchayat_code = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("sssisdddddiss", 
            $plan_name, $financial_year, $date,
            $bank_id, $payer_name, $account_balance,
            $balance_inhand, $amount_to_pay, $total_amount, $shera,
            $record_id, $district_code, $panchayat_code
        );
    } else {
        // Insert new record
        $query = "INSERT INTO bank_bharane 
                 (district_code, plan_name, financial_year, date, 
                 bank_id, payer_name, account_balance,
                 balance_inhand, amount_to_pay, total_amount, shera, panchayat_code)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ssssisddddss", 
            $district_code, $plan_name, $financial_year, $date,
            $bank_id, $payer_name, $account_balance,
            $balance_inhand, $amount_to_pay, $total_amount, $shera, $panchayat_code
        );
...
function handleDeleteRequest() {
    ...
    $query = "DELETE FROM bank_bharane WHERE id = ?";
    $stmt = $conn->prepare($query);
...
if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    header('Location: ../namuna10_dainandin_bank_bharne.php');
    exit;
}