...
if ($isUpdate) {
    $sql = "UPDATE water_tax SET 
        ...
        nov_reading=?, dec_reading=?, pani_prakar=?, total_amount=?,
        updated_at=NOW()
        WHERE id=?";
    ...
    $stmt->bind_param("ssssssssssssiii", 
        ...
        $_POST['pani_prakar'], $_POST['total_amount'], $existingId
    );
    $stmt->execute();
    if ($stmt->execute()) {
...
pani_prakar, total_amount, panchayat_code) VALUES (
?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, ?
)";

$stmt = $conn->prepare($sql);
$stmt->bind_param("issssssssssssssis", $malmatta_no, $year,
  ...
  $readings['nov'], $readings['dec'], $pani_prakar, $total_amount, $_SESSION['panchayat_code']
);
