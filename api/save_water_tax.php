...
if ($isUpdate) {
    $sql = "UPDATE water_tax SET 
        ...
        nov_reading=?, dec_reading=?, pani_prakar=?, total_amount=?, khasara_no=?,
        updated_at=NOW()
        WHERE id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssssssssssiisi", 
        ...
        $_POST['pani_prakar'], $_POST['total_amount'], $_POST['khasara_no'], $existingId
    );
    $stmt->execute();
    if ($stmt->execute()) {
...
pani_prakar, total_amount, panchayat_code, khasara_no) VALUES (
?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, ?,?
)";

$stmt = $conn->prepare($sql);
$stmt->bind_param("issssssssssssssiss", $malmatta_no, $year,
  ...
  $readings['nov'], $readings['dec'], $pani_prakar, $total_amount, $_SESSION['panchayat_code'], $khasara_no
);
